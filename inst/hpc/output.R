...
dd.protein.quants <- dd.protein.quants[, .(median = median(value) / log(2), mad = mad(value) / log(2)), by = .(ProteinID, SampleID)]
...
pca.assays <- prcomp(t(protein.quants[complete.cases(protein.quants),]),
                     center = T, scale = rowMeans(protein.quants.mads[complete.cases(protein.quants.mads),]^2))
rm(protein.quants.mads)
rm(protein.quants)
