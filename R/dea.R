...
dea_MCMCglmm <- function(
  ...
  execute_model <- function(DT.priors, chain, step, ...) {
    ...
    out <- foreach(DT.chunk = iterators::iter(DTs), .final = rbindlist_of_lists, .inorder = F, .packages = "data.table", .options.snow = list(progress = function(n) setTxtProgressBar(pb, (step-1)*length(DTs) + n))) %dorng% {
      ...
      out$DT.DIC <- rbindlist(lapply(output.chunk, function (output.protein) {
        rbindlist(lapply(output.protein, function (output.contrast) {
          if (!is.null(output.contrast$fit)) data.table(DIC = output.contrast$fit$DIC)
        }), idcol = "Model")
      }), idcol = "ProteinID")
      if (nrow(out$DT.DIC > 0)) {
        out$DT.DIC[, ProteinID := as.integer(ProteinID)]
  ...
  DT.de <- dcast(out$DT.DIC, Model + ProteinID ~ chainID, drop = F, value.var = "DIC")
  DT.de <- merge(DT.de, dcast(out$DT.rcov, Model + ProteinID ~ Effect, drop = F, value.var = c("v", "df")), by = c("Model", "ProteinID") )
