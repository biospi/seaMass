...
norm_theta <- function(
  ...
  message(paste0("[", Sys.time(), "]  standardised group quants..."))
  ...
  parallel_lapply(as.list(1:ctrl@model.nchain), function(item, fit, ctrl, input, output, DT) {
    ...
    DT.group.vars[, Group := factor(sub("^Group(.*)\\.units", "\\1", as.character(Group)), levels = levels(DT[, Group]))]

    # write groups vars
    setcolorder(DT.group.vars, "Group")
    setorder(DT.group.vars, Group, chainID, mcmcID)
    fst::write.fst(DT.group.vars, file.path(fit, output2, paste(item, "fst", sep = ".")))
    if (item == 1) fst::write.fst(DT.group.vars[, .(file = file.path(output2, "1.fst"), from = first(.I), to = last(.I)), by = Group], file.path(fit, paste(output2, "index.fst", sep = ".")))
