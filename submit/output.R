...
prefix <- ifelse(file.exists("quant"), ".", file.path("..", "..", "quant", "results", "quant"))
dir.create("normquants", showWarnings = F)
...
for (j in 1:nchain) {
  ...
  files <- list.files(prefix, paste0("^[0-9]+\\.", j, "\\.rds$"))
  ...
  if (length(files) > 0) {
    ...
    # read MCMC samps
    for (f in files) {
      mcmc.quants.chain <- readRDS(file.path(prefix, f))
      ps <- as.integer(sub("^([0-9]+)\\.[0-9]+\\.[0-9]+$", "\\1", colnames(mcmc.quants.chain)))
      bs <- as.integer(sub("^[0-9]+\\.([0-9]+)\\.[0-9]+$", "\\1", colnames(mcmc.quants.chain)))
      as <- as.integer(sub("^[0-9]+\\.[0-9]+\\.([0-9]+)$", "\\1", colnames(mcmc.quants.chain)))
      ...
      for (k in 1:ncol(mcmc.quants.chain)) {
        ...
        if (is.na(baseline.quants[ps[k], baseline.quants[ps[k], as[k]]])) {
          ...
          mcmc.quants[, ps[k], baseline.quants[ps[k], as[k]]] <- 0.0
        }
        
        mcmc.quants[, ps[k], as[k]] <- mcmc.quants.chain[, k]
      }
    }
  }
  ...
  for (a in 1:nA) saveRDS(as.mcmc(mcmc.quants[, !is.na(colSums(mcmc.quants[,, a])), a]), file.path("normquants", paste0(j, ".", a, ".rds")))
...
ggsave(paste0(dd.params[Key == "bayesprot.id", Value], "exposures.pdf"), g, width = 8, height = 0.5 * nA)
...
for (a in 1:nA) {
  ...
  for (j in 1:nchain) {
    mcmc.quants[[j]] <- readRDS(file.path("normquants", paste0(j, ".", a, ".rds")))
  }
...
# stats for variances
prefix <- ifelse(file.exists("sd"), ".", file.path("..", "..", "quant", "results", "sd"))
...
for (i in 1:nbatch) {
  for (j in 1:nchain) {
    message("[", paste0(Sys.time(), " Reading stdevs batch ", i, "/", nbatch, " chain ", j, "/", nchain, "...]"))
  
    load(file.path(prefix, paste0(i, ".", j, ".Rdata")))
